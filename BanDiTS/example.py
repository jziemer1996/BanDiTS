...
from BanDiTS.water_functions import *
...
def main():
    ...
    statistical_functions = [count_threshold]
    statistical_args = [{"lower":-23.45, "upper":-22}]         # take care of name of statistical arguments in export (line 113)
    ...
    breakpoint_functions = [count_threshold2]
    breakpoint_args = [{"lower":-24, "upper":-22}]

    ######################   NO USER INPUT BEYOND THIS POINT   ###############################

    return raster_folder, raster_filename, output_folder, filter_functions, filter_args, statistical_functions, \
        statistical_args, water_functions, water_args, breakpoint_functions, breakpoint_args
...
def statistics_func(raster_folder, raster_filename, output_folder, statistical_functions, statistical_args):
    ...
    for i, func in enumerate(statistical_functions):
        ...
        # exporting result to new raster
        export_arr.functions_out_array(outname=outname + "_" + func_name + str(4), arr=result, input_file=input_raster,
                                       dtype=dtype)
...
if __name__ == '__main__':
    ...
    water_func(raster_folder=str(in_variables[0]), raster_filename=str(in_variables[1]),
                    output_folder=str(in_variables[2]), water_functions=in_variables[7],
                    water_args=in_variables[8])
    ...
    #                 output_folder=str(in_variables[2]), breakpoint_functions=in_variables[9],
    #                 breakpoint_args=in_variables[10])
